...
export function meta(_args: Route.MetaArgs) {
  return [
    { title: "Instrument - Piano Roll & Hybrid Synthesizer - Studio" },
    {
      name: "description",
      content:
...
export default function Instrument() {
  ...
  const track = useMemo(() => {
    if (instrumentId) {
      const found = studio.song.tracks.find((t) => t.id === instrumentId);
      if (found) return found;
    }
    return getActiveTrack(studio);
  }, [studio, instrumentId]);

  const activeNotes = useMemo(() => new Set(track.notes), [track.notes]);
  const setActiveNotes = useCallback(
    (updater: Set<string> | ((prev: Set<string>) => Set<string>)) => {
      setStudio((prev) => {
        const currentSet = new Set(track.notes);
        const nextSet =
          typeof updater === "function" ? updater(currentSet) : updater;
        return updateTrack(prev, track.id, { notes: Array.from(nextSet) });
      });
    },
    [setStudio, track.id, track.notes],
  );

  const disabledNotes = useMemo(
    () => new Set(track.disabledNotes),
    [track.disabledNotes],
  );
  const setDisabledNotes = useCallback(
    (updater: Set<string> | ((prev: Set<string>) => Set<string>)) => {
      setStudio((prev) => {
        const currentSet = new Set(track.disabledNotes);
        const nextSet =
          typeof updater === "function" ? updater(currentSet) : updater;
        return updateTrack(prev, track.id, {
          disabledNotes: Array.from(nextSet),
        });
      });
    },
    [setStudio, track.id, track.disabledNotes],
  );

  const noteVelocities = track.noteVelocities;
      ...
      setStudio((prev) => {
        const nextVel =
          typeof updater === "function"
            ? updater(track.noteVelocities)
            : updater;
        return updateTrack(prev, track.id, { noteVelocities: nextVel });
      });
    },
    [setStudio, track.id, track.noteVelocities],
  );
  ...
  const setIsLooping = useCallback(
    (updater: boolean | ((prev: boolean) => boolean)) => {
      setStudio((prev) => ({
        ...prev,
        isLooping:
          typeof updater === "function" ? updater(prev.isLooping) : updater,
      }));
    },
    [setStudio],
  );
  ...
  const setVolume = useCallback(
    (val: number | ((prev: number) => number)) => {
      setStudio((prev) => {
        const nextVol = typeof val === "function" ? val(track.volume) : val;
        return updateTrack(prev, track.id, { volume: nextVol });
      });
    },
    [setStudio, track.id, track.volume],
  );

  const [currentStep, setCurrentStep] = useState(0);
  const totalSteps = track.totalSteps;
  const setTotalSteps = useCallback(
    (steps: number | ((prev: number) => number)) => {
      setStudio((prev) => {
        const nextSteps =
          typeof steps === "function" ? steps(track.totalSteps) : steps;
        return updateTrack(prev, track.id, { totalSteps: nextSteps });
      });
    },
    [setStudio, track.id, track.totalSteps],
  );

  const timeSignature =
    track.timeSignature ?? studio.song.timeSignature ?? "4/4";
  const setTimeSignature = useCallback(
    (sig: "4/4" | "3/4" | "triplet") => {
      setStudio((prev) => updateTrack(prev, track.id, { timeSignature: sig }));
    },
    [setStudio, track.id],
  );

  const bpm = studio.song.bpm ?? studio.bpm;
  const setBpm = useCallback(
    (val: number | ((prev: number) => number)) => {
      setStudio((prev) => {
        const nextBpm = typeof val === "function" ? val(prev.bpm) : val;
        return {
          ...prev,
          bpm: nextBpm,
          song: {
  ...
  const selectedPreset = track.preset;
  ...
  const playerView = track.playerView;
  const setPlayerView = useCallback(
    (
      ...
      setStudio((prev) => {
        const nextView =
          typeof val === "function" ? val(track.playerView) : val;
        return updateTrack(prev, track.id, { playerView: nextView });
      });
    },
    [setStudio, track.id, track.playerView],
  );
  ...
  const handlePresetChange = useCallback(
    (preset: string) => {
      ...
      setStudio((prev) => {
        const updated = updateTrack(prev, track.id, {
          preset,
  ...
  }, [isPlaying, bpm, timeSignature, triggerStepNotes]);
  ...
  return (
    <div className="h-screen w-screen max-w-full overflow-hidden bg-surface text-font dark:bg-surface-dark dark:text-surface flex flex-col font-sans transition-colors duration-200">
      ...
      <main className="flex-1 min-h-0 w-full overflow-hidden flex flex-col p-0 m-0 border-b-2 border-stone-300 dark:border-stone-800">
        <PianoRoll
          ...
          scale={studio.scale}
          onScaleChange={(sc) => setStudio((prev) => ({ ...prev, scale: sc }))}
        />
      </main>

      <footer className="h-[270px] sm:h-[280px] min-h-0 w-full flex flex-row gap-2.5 p-2 bg-stone-100/60 dark:bg-[#06080c] overflow-x-auto overflow-y-hidden flex-shrink-0">
            ...
            rightHeaderSlot={
              isPlayerCollapsed ? (
                <Button
                  ...
                  className="self-center h-7 w-7 bg-white dark:bg-[#0a0d14] border border-stone-200 dark:border-[#1f2533] hover:border-stone-400 dark:hover:border-[#38435d] hover:bg-stone-50 dark:hover:bg-[#111520] text-stone-700 dark:text-stone-300 hover:text-stone-900 dark:hover:text-white rounded-lg flex items-center justify-center flex-shrink-0 shadow-sm transition-all cursor-pointer select-none group p-0"
                >
                  {playerView === "drums" ? (
                    <Drum className="w-3.5 h-3.5 text-stone-600 dark:text-stone-400 group-hover:text-stone-900 dark:group-hover:text-white transition-colors" />
                  ) : (
                    <Piano className="w-3.5 h-3.5 text-stone-600 dark:text-stone-400 group-hover:text-stone-900 dark:group-hover:text-white transition-colors" />
                  )}
                </Button>
              ) : undefined
            }
